...
`+100 HP`. It currently has no normal production acquisition: it is absent from crafting and
ordinary production loot. Developer spawning remains a debug-only path, and copies already present
in saved inventories remain valid.
...
rules remain open, including a future normal source for Basic Rune.
