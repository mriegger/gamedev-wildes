...
coordinator evaluates inventory, equipped armor, and killer affinity history for that update, while
`Game` snapshots whether a slime was attached before defeat cleanup removes attachments.
Weaponless guidance has absolute priority; otherwise one applicable recovery, armor, affinity,
attached-slime, or crowd tip is selected randomly, with crowd guidance suppressed when the killing
enemy was hit with a hammer and Progression guidance as the fallback. The death screen owns only
centered rich-text presentation, while affinity wording and colors reuse the shared combat formatter
and palette.
