...
Melee definitions own their idle and attack transforms, animation style, and two-handed stance. This lets the copper sword retain its one-handed alternating sweep while the copper hammer holds a custom modeled handle in both hands, raises it overhead, and drives it downward without adding weapon-specific branches to inventory selection. Switching away from a melee item cancels its presentation before the newly selected tool is rendered.
