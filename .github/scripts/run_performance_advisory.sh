...
benchmark_id="${4:?benchmark id required}"
repository_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
base_dir="$output_dir/base"
candidate_dir="$output_dir/candidate"
collection_issues="$output_dir/collection-issues-$benchmark_id.md"
...
run_benchmark() {
  ...
  printf 'Running %s %s run %s\n' "$revision" "$benchmark_id" "$run_name"
  timeout 300 godot --headless --path "$project_dir" --script "res://tests/$script_name" -- "--output=$report_path" > "$log_path" 2>&1
  status=$?
  cat "$log_path"
  if [ "$status" -ne 0 ]; then
...
run_selected_benchmark() {
  ...
  case "$benchmark_id" in
    entity_efficiency)
      run_benchmark "$revision" "$project_dir" "$benchmark_id" entity_efficiency_benchmark.gd "ENTITY_EFFICIENCY PASS" "$run_name" "$result_dir"
      ;;
    game_performance)
      run_benchmark "$revision" "$project_dir" "$benchmark_id" game_performance_benchmark.gd "GAME_PERFORMANCE PASS" "$run_name" "$result_dir"
      ;;
  esac
}
...
if [ "$benchmark_id" != entity_efficiency ] && [ "$benchmark_id" != game_performance ]; then
  emit_warning "Performance telemetry unavailable" "Unsupported benchmark $benchmark_id"
  exit 0
fi
...
for pair in 1 2 3; do
  if [ $((pair % 2)) -eq 1 ]; then
    if [ "$baseline_available" -eq 1 ]; then
      run_selected_benchmark baseline "$baseline_project_dir" "$pair" "$base_dir"
    fi
    if [ "$candidate_available" -eq 1 ]; then
      run_selected_benchmark candidate "$candidate_project_dir" "$pair" "$candidate_dir"
    fi
  else
    if [ "$candidate_available" -eq 1 ]; then
      run_selected_benchmark candidate "$candidate_project_dir" "$pair" "$candidate_dir"
