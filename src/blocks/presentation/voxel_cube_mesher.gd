...
func build_mesh_data(solid_cells: Array[Vector3i], block_query: Callable, face_open_query: Callable) -> Variant:
	assert(block_query.is_valid())
	assert(face_open_query.is_valid())
	...
	for cell in solid_cells:
		var block_id := int(block_query.call(cell))
		assert(BlockId.is_chunk_cube(block_id))
		for direction in FACE_DIRECTIONS:
			if not bool(face_open_query.call(cell + direction)):
				continue
			_append_face(
				cell,
				direction,
