...
@export_range(2, 8, 1) var minimum_decorative_apples: int = 2
@export_range(2, 8, 1) var maximum_decorative_apples: int = 8
...
func validate(item_catalog: ItemCatalog) -> bool:
	return (
		...
		and minimum_decorative_apples >= 2
		and minimum_decorative_apples <= maximum_decorative_apples
		and maximum_decorative_apples <= 8
