...
static func is_valid_movement_speed(value: float) -> bool:
...
func validate(source: String) -> bool:
	var valid := true
	if not is_valid_movement_speed(movement_speed):
		push_error("[StoneGolemBehaviorDefinition] Invalid movement speed at %s" % source)
		valid = false
