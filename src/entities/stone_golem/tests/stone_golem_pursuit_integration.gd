...
func _test_last_seen_pursuit(definition: EntityDefinition, world: VoxelWorld) -> void:
	...
	actor.tick_gameplay(0.0, _observation(outside_detection), Vector3(4.0, 0.0, 0.0), budget)
	...
	budget.reset()
	actor.tick_gameplay(VoxelPlayerVisibilitySensor.SAMPLE_INTERVAL_SECONDS, _observation(visible_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(VoxelPlayerVisibilitySensor.SAMPLE_INTERVAL_SECONDS * 0.5, _observation(hidden_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(VoxelPlayerVisibilitySensor.SAMPLE_INTERVAL_SECONDS * 0.5, _observation(moved_hidden_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.5, _observation(moved_hidden_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(behavior.target_memory_seconds, _observation(moved_hidden_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.125, _observation(restored_visible_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.1, _observation(beyond_forget), Vector3(4.0, 0.0, 0.0), budget)
...
func _test_bounded_navigation_recovery(definition: EntityDefinition, world: VoxelWorld) -> void:
	...
	actor.tick_gameplay(0.125, _observation(unreachable_goal), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.5, _observation(reachable_goal), Vector3.ZERO, budget)
...
func _test_obstacle_detour(definition: EntityDefinition, world: VoxelWorld) -> void:
	...
	var budget := NavigationSearchBudget.new(2)
	actor.tick_gameplay(0.0, _observation(target), Vector3.ZERO, budget)
	...
	for _step in range(20):
		budget.reset()
		actor.tick_gameplay(0.1, _observation(target), Vector3.ZERO, budget)
...
func _test_rotated_budget_and_crowd_separation(definition: EntityDefinition, world: VoxelWorld) -> void:
	...
	runtime.setup(catalog, world, 3, 3, EntityNavigationLimits.new(32, 512, 2), EntityRuntime.Mode.GAMEPLAY)
