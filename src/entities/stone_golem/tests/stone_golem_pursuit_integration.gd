...
func _test_last_seen_pursuit(definition: EntityDefinition, world: VoxelWorld) -> void:
	...
	actor.tick(0.1, _observation(outside_detection), Vector3(4.0, 0.0, 0.0), budget)
	...
	budget.reset()
	actor.tick(0.1, _observation(visible_player), Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick(0.125, _observation(hidden_player), Vector3.ZERO, budget)
	_expect(actor.brain.get_movement_goal().is_equal_approx(retained_goal), "hidden player replaced the last-seen pursuit goal")
