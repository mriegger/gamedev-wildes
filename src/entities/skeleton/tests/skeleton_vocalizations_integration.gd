...
func _expect(condition: bool, message: String):
	if condition:
		return
	_failures += 1
	push_error("[skeleton_vocalizations_integration] FAIL: %s" % message)
...
func _run():
	...
	actor.setup(13, definition, world, 31415, EntityNavigationLimits.new(24, 256, 1))
	var vocalizations := actor.get_node_or_null(actor.vocalizations_path) as EntityVocalizations
	_expect(vocalizations != null, "skeleton vocalizations missing")
	var profile := vocalizations.profile
	_expect(profile != null and profile.validate(), "skeleton vocalization profile invalid")
	...
	_expect(profile.streams.size() == 3, "expected 3 vocalizations, got %d" % profile.streams.size())
	...
	if _failures == 0:
		print("SKELETON_VOCALIZATIONS PASS orphan=%d" % orphan_count)
		quit(0)
	else:
		print("SKELETON_VOCALIZATIONS FAIL failures=%d" % _failures)
		quit(1)
