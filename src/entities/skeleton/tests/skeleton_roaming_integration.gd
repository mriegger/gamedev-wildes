...
func _test_rotated_crowd_navigation(definition: EntityDefinition) -> void:
	...
	runtime.setup(catalog, _make_world(), 3, 3, EntityNavigationLimits.new(32, 512, 2), EntityRuntime.Mode.GAMEPLAY)
	...
	for tick_index in range(3):
		...
		runtime.tick_gameplay(0.05, observation)
...
func _run() -> void:
	...
	for _step in range(8):
		budget.reset()
		actor.tick_gameplay(0.1, distant_observation, Vector3.ZERO, budget)
	...
	actor._path_follower.request_repath()
	budget.reset()
	actor.tick_gameplay(0.0, distant_observation, Vector3.ZERO, budget)
	...
	for _step in range(8):
		budget.reset()
		actor.tick_gameplay(0.1, nearby_observation, Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.124, rotated_observation, Vector3.ZERO, budget)
	_expect(actor.brain.state == SkeletonBrain.State.HIDE, "Camera change invalidated cover before the 0.125-second cadence")
	budget.reset()
	budget.try_acquire()
	budget.try_acquire()
	actor.tick_gameplay(0.001, rotated_observation, Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.0, nearby_observation, Vector3.ZERO, budget)
	...
	budget.reset()
	actor.tick_gameplay(0.124, nearby_observation, Vector3.ZERO, budget)
	_expect(actor.brain.state == SkeletonBrain.State.HIDE, "Mined cover invalidated hide before the 0.125-second cadence")
	budget.reset()
	actor.tick_gameplay(0.001, nearby_observation, Vector3.ZERO, budget)
	...
	for _step in range(100):
		sprint_budget.reset()
		sprint_actor.tick_gameplay(0.0, sprint_observation, Vector3.ZERO, sprint_budget)
	...
	sprint_budget.reset()
	sprint_actor.tick_gameplay(0.999, sprint_observation, Vector3.ZERO, sprint_budget)
	...
	sprint_budget.reset()
	sprint_actor.tick_gameplay(0.001, sprint_observation, Vector3.ZERO, sprint_budget)
	...
	sprint_budget.reset()
	sprint_actor.tick_gameplay(0.1, sprint_observation, Vector3.ZERO, sprint_budget)
	...
	sprint_budget.reset()
	sprint_actor.tick_gameplay(0.0, outside_observation, Vector3.ZERO, sprint_budget)
	...
	stale_budget.reset()
	stale_actor.tick_gameplay(0.0, stale_initial_observation, Vector3.ZERO, stale_budget)
	_expect(stale_actor.brain.is_cover_search_in_progress(), "Stale-cover fixture did not begin an incremental search")
	for _step in range(40):
		stale_budget.reset()
		stale_actor.tick_gameplay(0.0, stale_latest_observation, Vector3.ZERO, stale_budget)
	...
	result_budget.reset()
	result_actor.tick_gameplay(0.1, sprint_observation, Vector3.ZERO, result_budget)
