...
func _init():
	...
	_expect(leveled_stats.get_level() == 3 and leveled_stats.get_experience() == 50, "level or carried experience is incorrect")
	...
	_expect(overflow_safe_stats.get_level() == 2 and overflow_safe_stats.get_experience() == 0, "maximum integer experience corrupted progression")
	...
	_expect(restored_stats.get_level() == 3 and restored_stats.get_experience() == 50, "restored progression changed")
	var hp_stats := ActorStats.new(player_definition)
	var depletion_count: Array[int] = [0]
	var health_changes: Array[Vector2] = []
	hp_stats.health_depleted.connect(func(): depletion_count[0] += 1)
	hp_stats.health_changed.connect(func(current_hp: float, maximum_hp: float): health_changes.append(Vector2(current_hp, maximum_hp)))
	...
	_expect(health_changes == [Vector2(70.0, 100.0), Vector2(100.0, 100.0), Vector2(0.0, 100.0), Vector2(1.0, 100.0), Vector2(0.0, 100.0), Vector2(1.0, 100.0), Vector2(0.0, 100.0)], "current HP changes did not emit exactly once with committed values")
	...
	var maximum_signal_stats := ActorStats.new(player_definition)
	var maximum_health_changes: Array[Vector2] = []
	maximum_signal_stats.health_changed.connect(func(current_hp: float, maximum_hp: float): maximum_health_changes.append(Vector2(current_hp, maximum_hp)))
	_expect(maximum_signal_stats.set_base_value(&"hp", 120.0), "maximum HP base change failed")
	_expect(maximum_signal_stats.set_base_value(&"hp", 120.0), "equivalent maximum HP base change failed")
	_expect(maximum_signal_stats.set_base_value(&"strength", 12.0), "unrelated base stat change failed")
	var maximum_hp_bonus := _modifier(&"maximum_hp_bonus", &"maximum_hp_bonus", &"hp", StatModifier.Operation.ADD, 20.0)
	_expect(maximum_signal_stats.add_modifier(maximum_hp_bonus), "maximum HP modifier failed")
	_expect(maximum_signal_stats.remove_modifier(maximum_hp_bonus.id), "maximum HP modifier removal failed")
	_expect(maximum_health_changes == [Vector2(100.0, 120.0), Vector2(100.0, 140.0), Vector2(100.0, 120.0)], "maximum-only HP changes did not emit exactly once with committed values")
