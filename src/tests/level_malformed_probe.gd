...
func _init() -> void:
	...
	var exposed_socket := source_catalog.get_module(&"dungeon_compact_dead_end").duplicate(true) as LevelModuleDefinition
	var exposed_perimeter := exposed_socket.sockets[0].cell + Vector3i.UP * 2
