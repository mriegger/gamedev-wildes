...
const MAX_REFILL_ATTEMPTS_PER_REGION: int = 256
...
const SHEEP_OFFSETS: Array[Vector2i] = [Vector2i(-14, 8), Vector2i(-7, 14), Vector2i(7, 14), Vector2i(14, 8)]
const SLIME_OFFSETS: Array[Vector2i] = [Vector2i(0, 18)]
const WATCHER_OFFSETS: Array[Vector2i] = [Vector2i(-18, -12), Vector2i(18, -12), Vector2i(0, -20)]
...
var _largest_observed_runtime_id: int = 0
...
func _assert_runtime_ids(actors: Array[EntityActor], context: String) -> void:
	...
	for actor in actors:
		...
		if _instance_by_runtime_id.has(runtime_id):
			...
			new_runtime_ids.append(runtime_id)
	new_runtime_ids.sort()
	for runtime_id in new_runtime_ids:
		_expect(runtime_id > _largest_observed_runtime_id, "%s observed non-monotonic runtime ID %d after %d" % [context, runtime_id, _largest_observed_runtime_id])
		_largest_observed_runtime_id = maxi(_largest_observed_runtime_id, runtime_id)
...
func _assert_population(coordinator: WorldEntityCoordinator, catalog: EntityCatalog, player_position: Vector3, time_of_day: float, context: String) -> void:
	...
	for actor in actors:
		...
		if actor.definition.ambient_despawn_outside_spawn_phase:
			var actor_spawns_during_day := actor.definition.ambient_spawn_phase == EntityDefinition.SpawnPhase.DAY
			_expect(is_day == actor_spawns_during_day, "%s retained out-of-phase runtime ID %d" % [context, actor.runtime_id])
	for definition in catalog.definitions:
...
func _assert_mixed_night_population(coordinator: WorldEntityCoordinator, catalog: EntityCatalog, context: String) -> void:
	...
	_expect(night_ids.size() == 5 and night_ids.has(&"zombie") and night_ids.has(&"skeleton") and night_ids.has(&"stone_golem") and night_ids.has(&"slime_large") and night_ids.has(&"watcher"), "%s night species IDs changed" % context)
	...
	for definition_id in [&"zombie", &"skeleton", &"stone_golem", &"sheep", &"slime_large", &"watcher"]:
		...
		for index in range(offsets.size()):
			...
			requests.append(EntitySpawnRequest.new(
				definition_id,
				player_position + Vector3(float(offset.x), 0.0, float(offset.y)),
				region_index * 100 + requests.size() + 1,
			))
	var runtime_ids := runtime.try_spawn_batch(requests)
	_expect(runtime_ids.size() == WorldEntityCoordinator.MAX_TOTAL_ACTIVE, "region %d mixed workload spawn was rejected" % region_index)
	_assert_population(coordinator, catalog, player_position, NIGHT_TIME, "region %d prepared night workload" % region_index)
...
func _arrange_active_population(coordinator: WorldEntityCoordinator, player_position: Vector3) -> Dictionary:
	...
	for actor in actors:
		...
		match definition_id:
			...
			&"watcher":
				...
				_expect(watcher != null, "active workload Watcher used the wrong actor type")
				if watcher != null:
	...
	_expect(int(next_index[&"sheep"]) == SHEEP_OFFSETS.size(), "active workload did not arrange four Sheep")
	_expect(int(next_index[&"slime_large"]) == SLIME_OFFSETS.size(), "active workload did not arrange one large Slime")
	_expect(int(next_index[&"watcher"]) == WATCHER_OFFSETS.size(), "active workload did not arrange three Watchers")
	return activity
...
func _actor_has_navigation_path(actor: EntityActor) -> bool:
	match actor.definition.id:
		...
		&"slime_large":
			return not (actor as SlimeActor)._path_follower._path.is_empty()
		&"watcher":
			return not (actor as WatcherActor)._path_follower._path.is_empty()
	return false
...
func _run() -> void:
	...
	for region_index in range(STREAM_REGIONS.size()):
		...
		for refill_cycle in range(MAX_REFILL_ATTEMPTS_PER_REGION):
			...
			_assert_path_budget(world, catalog, _ready_region, NIGHT_TIME, cycle, context)
	...
	_expect(_instance_by_runtime_id.size() >= STREAM_REGIONS.size() * WorldEntityCoordinator.MAX_TOTAL_ACTIVE, "soak observed only %d unique runtime IDs" % _instance_by_runtime_id.size())
	...
	_expect(int(_full_path_acquisitions_by_species[&"slime_large"]) == expected_full_regions * SLIME_OFFSETS.size(), "full workloads did not record every Slime path acquisition")
	_expect(int(_full_path_acquisitions_by_species[&"watcher"]) == expected_full_regions * WATCHER_OFFSETS.size(), "full workloads did not record every Watcher path acquisition")
