...
func _run() -> void:
	...
	_expect(coordinator._retiring.has(target_id), "lethal damage did not retain the zombie for fade-out")
	contact_count_before = _contacts.size()
	zombie_actor._advance_melee_contact(zombie_profile.contact_time + 0.01)
	...
	coordinator.tick(EntityCoordinator.SPAWN_INTERVAL_SECONDS, player.global_position, 20.0)
	_expect(coordinator.get_active_count() == 1, "lethal retirement did not allow replacement spawning")
...
func _test_sheep_damage(world: VoxelWorld, sword_profile: MeleeAttackProfile) -> void:
	...
	_expect(coordinator._retiring.has(target_id), "dead sheep did not enter fade-out retirement")
