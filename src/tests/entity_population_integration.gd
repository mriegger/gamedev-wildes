...
func _expect_index_bounded(coordinator: WorldEntityCoordinator, context: String) -> void:
	var active_count := coordinator.get_runtime().get_active_count()
	var index := coordinator.get_runtime()._spatial_index as EntitySpatialIndex
...
func _index_actors(coordinator: WorldEntityCoordinator, actors: Array[EntityActor]) -> void:
	for actor in actors:
...
func _run() -> void:
	...
	_expect(coordinator.get_runtime()._prepared_actor_count() == catalog.definitions.size(), "setup did not prepare one actor per definition")
	...
	for index in range(spawn_positions.size()):
		...
		_expect(horizontal_distance >= WorldEntityCoordinator.MIN_SPAWN_DISTANCE, "spawn %d was inside the minimum annulus at %.3f" % [index, horizontal_distance])
		_expect(horizontal_distance <= WorldEntityCoordinator.MAX_SPAWN_DISTANCE, "spawn %d exceeded the maximum annulus at %.3f" % [index, horizontal_distance])
	...
	if actors.size() == 6:
		...
		var first_separation := coordinator.get_runtime()._get_separation_velocity(actors[0])
	...
	_expect(coordinator.get_runtime()._prepared_actors.is_empty(), "shutdown retained prepared actors")
