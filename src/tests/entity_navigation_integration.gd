...
func _test_ground_melee_enemy_brain_transitions() -> void:
	...
	var mid_swing_target := Vector3(4.0, 0.0, 2.0)
	attacker.advance(0.25, self_position, mid_swing_target, true, true)
	_expect(attacker._last_seen_position == mid_swing_target, "fresh mid-swing sample was not retained")
	_expect(attacker.state == GroundMeleeEnemyBrain.State.ATTACK, "attack ended before its duration")
	attacker.advance(0.3, self_position, mid_swing_target, true, false)
	_expect(attacker.state == GroundMeleeEnemyBrain.State.ATTACK, "attack state was not retained on its completion tick")
	attacker.advance(0.01, self_position, mid_swing_target, false, true)
	_expect(attacker.get_movement_goal() == mid_swing_target, "post-attack chase did not use the fresh mid-swing target")
...
func _test_zombie_cached_visibility_keeps_last_sampled_goal() -> void:
	...
	var search_budget := NavigationSearchBudget.new(1)
	actor.tick_gameplay(0.0, EntityTargetObservation.create(sampled_target, sampled_target, Vector3.FORWARD, Vector3.RIGHT), Vector3.ZERO, search_budget)
	...
	var half_interval := VoxelPlayerVisibilitySensorType.SAMPLE_INTERVAL_SECONDS * 0.5
	search_budget.reset()
	actor.tick_gameplay(half_interval, EntityTargetObservation.create(hidden_position, hidden_position, Vector3.FORWARD, Vector3.RIGHT), Vector3.ZERO, search_budget)
...
func _test_zombie_actor_movement_and_animation() -> void:
	...
	for _step in range(5):
		search_budget.reset()
		actor.tick_gameplay(0.1, EntityTargetObservation.create(target, target, Vector3.FORWARD, Vector3.RIGHT), Vector3.ZERO, search_budget)
	...
	search_budget.reset()
	actor.tick_gameplay(0.01, EntityTargetObservation.create(target, target, Vector3.FORWARD, Vector3.RIGHT), Vector3.ZERO, search_budget)
	...
	var swing_target := actor.global_position + Vector3(4.0, 0.0, 2.0)
	var swing_observation := EntityTargetObservation.create(swing_target, swing_target, Vector3.FORWARD, Vector3.RIGHT)
	search_budget.reset()
	actor.tick_gameplay(VoxelPlayerVisibilitySensorType.SAMPLE_INTERVAL_SECONDS, swing_observation, Vector3.ZERO, search_budget)
	_expect(actor._visibility_sensor.did_sample_line_of_sight(), "attacking Zombie did not perform its fresh LOS sample")
	_expect(actor.brain.state == GroundMeleeEnemyBrain.State.ATTACK, "fresh mid-swing sample interrupted the Zombie attack")
	_expect(actor.brain._last_seen_position.is_equal_approx(swing_target), "attacking Zombie did not retain its fresh sampled target")
	search_budget.reset()
	var remaining_attack := actor._behavior.melee_profile.duration - VoxelPlayerVisibilitySensorType.SAMPLE_INTERVAL_SECONDS
	actor.tick_gameplay(remaining_attack, swing_observation, Vector3.ZERO, search_budget)
	_expect(actor.brain.state == GroundMeleeEnemyBrain.State.ATTACK, "Zombie attack did not retain its completion tick")
	search_budget.reset()
	actor.tick_gameplay(0.001, swing_observation, Vector3.ZERO, search_budget)
	_expect(actor.brain.state == GroundMeleeEnemyBrain.State.CHASE, "Zombie did not chase after its attack completed")
	_expect(actor.brain.get_movement_goal().is_equal_approx(swing_target), "Zombie post-attack chase did not use its mid-swing sample")
