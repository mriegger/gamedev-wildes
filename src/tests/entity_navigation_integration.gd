...
func _test_zombie_actor_movement_and_animation() -> void:
	...
	search_budget.reset()
	actor.tick(VoxelPlayerVisibilitySensorType.SAMPLE_INTERVAL_SECONDS, swing_observation, Vector3.ZERO, search_budget)
	...
	actor.tick(remaining_attack, swing_observation, Vector3.ZERO, search_budget)
	_expect(actor.brain.state == GroundMeleeEnemyBrain.State.ATTACK, "Zombie attack did not retain its completion tick")
	search_budget.reset()
	actor.tick(0.001, swing_observation, Vector3.ZERO, search_budget)
