...
func _run():
	...
	_expect(bird.combat_targetable and bird.experience_reward == 0, "bird combat targeting or progression policy changed")
	_expect(is_equal_approx(bird.stats_definition.maximum_hp, 1.0), "bird maximum HP is not one")
	...
	_expect(owl.combat_targetable and owl.experience_reward == 0, "owl combat targeting or progression policy changed")
	_expect(is_equal_approx(owl.stats_definition.maximum_hp, 1.0), "owl maximum HP is not one")
	...
	var night_count := (
		...
		+ coordinator.get_runtime().get_definition_count(&"skeleton")
		+ coordinator.get_runtime().get_definition_count(&"slime_large")
		+ coordinator.get_runtime().get_definition_count(&"watcher")
		+ coordinator.get_runtime().get_definition_count(&"stone_golem")
	)
