...
func set_draw_pose(active: bool, raise_progress: float, draw_progress: float, full_draw_distance: float, arrow_scene: PackedScene, actor_forward: Vector3) -> void:
	if not active:
		reset_draw_pose()
		return
	assert(arrow_scene != null)
	assert(actor_forward.is_finite())
	assert(is_finite(full_draw_distance) and full_draw_distance > 0.0 and full_draw_distance <= 1.0)
	var forward := actor_forward
	forward.y = 0.0
	...
	var target_basis := Basis(actor_right, -forward, Vector3.UP)
