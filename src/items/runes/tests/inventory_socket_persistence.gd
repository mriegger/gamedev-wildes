...
func _test_identity_and_persistence(item_catalog: ItemCatalog) -> void:
	...
	_expect(inventory.add_stack(InventoryStack.new(&"copper_sword", 1, affixed)), "affixed sword was not added")
	_expect(inventory.add_stack(InventoryStack.new(&"copper_sword", 1, plain)), "plain sword was not added")
