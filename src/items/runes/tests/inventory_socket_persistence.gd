...
func _init() -> void:
	...
	_expect(version_five_save["version"] == 6, "version-five save has the wrong migrated version")
	for region_name in ["hotbar", "backpack", "equipment"]:
	...
	_expect(version_four_save["version"] == 6 and version_four_save["item_proficiency"] == {}, "version-four migration chain lost progression shape")
