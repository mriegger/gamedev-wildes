...
func poll_async(max_to_apply: int = 4) -> int:
	if _job_queue.is_empty() and _result_queue.is_empty() and _pending_water.is_empty():
		return 0
	...
	# Drain any pending water from previous frames first.
	# Water creation is cheap and should not consume the terrain budget.
	...
	while applied < max_to_apply and Time.get_ticks_msec() - time_start < time_budget_ms:
		...
		if mesh == null and water_mesh == null:
			...
			if water_mesh != null:
				# Prefer applying water immediately in the same frame to avoid
				# an extra frame of latency. Fall back to pending queue if out of budget.
	...
	# Drain any water that was queued during this call if we still have budget.
	while not _pending_water.is_empty() and Time.get_ticks_msec() - time_start < time_budget_ms:
