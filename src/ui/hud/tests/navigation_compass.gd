...
func _run() -> void:
	...
	_expect(is_equal_approx(compass._get_target_relative_bearing(), 90.0), "east target was not right of north-facing camera")
	...
	_expect(is_zero_approx(compass._get_target_relative_bearing()), "east target was not centered for east-facing camera")
	...
	_expect(is_zero_approx(compass._get_target_relative_bearing()), "target at the tracked position did not center")
