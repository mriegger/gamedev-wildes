...
func _run() -> void:
	...
	_expect(toolbelt.get_placeable_item_ids().size() == expected_placeables.size(), "placeable query exposed mutable state")
...
func _test_generic_ui(item_catalog: ItemCatalog, toolbelt: CreativeToolbelt) -> void:
	...
	_expect(palette_grid.get_child_count() == toolbelt.get_placeable_item_ids().size(), "palette did not contain every placeable item")
