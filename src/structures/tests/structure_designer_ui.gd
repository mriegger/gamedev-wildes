...
func _run() -> void:
	...
	_expect(expected_placeables.size() == 14, "item catalog did not expose the expected 14 placeables")
	...
	_expect(toolbelt.get_placeable_item_ids().size() == 14, "placeable query exposed mutable state")
...
func _test_generic_ui(item_catalog: ItemCatalog, toolbelt: CreativeToolbelt) -> void:
	...
	_expect(palette_grid.get_child_count() == 14, "palette did not contain every placeable item")
