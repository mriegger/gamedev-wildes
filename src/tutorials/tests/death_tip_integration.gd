...
func _test_priority_and_recovery_tips() -> void:
	...
	_expect(coordinator.choose_tip(5) == DeathTipCoordinator.NO_WEAPON_TIP, "weaponless death did not take priority over every other tip")
	_add_item(fixture, &"copper_sword")
	var tips := coordinator.get_applicable_tips(0)
	...
	tips = coordinator.get_applicable_tips(0)
	...
	_expect(coordinator.get_applicable_tips(0) == tips, "consuming an unrelated item suppressed recovery guidance")
	...
	_expect(coordinator.get_applicable_tips(0) == [DeathTipCoordinator.PROGRESSION_TIP], "fully prepared death did not fall back to the progression tip")
...
func _test_resistance_and_crowd_tips() -> void:
	...
	var tips := coordinator.get_applicable_tips(4)
	...
	tips = coordinator.get_applicable_tips(4)
	...
	_expect(coordinator.get_applicable_tips(0) == [DeathTipCoordinator.PROGRESSION_TIP], "entity-context reset retained a stale killer tip")
