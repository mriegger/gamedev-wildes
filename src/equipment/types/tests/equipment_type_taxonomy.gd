...
func _init() -> void:
	...
	var bow_type := item_catalog.get_equipment_type(&"weapon_bow")
	...
	_expect(bow_type.parent == weapon_type, "bow type has the wrong parent")
	...
	_expect(bow.equipment_type == bow_type, "bow does not use the canonical bow type")
	...
	_expect(basic_rune.is_compatible_with(bow), "basic rune rejected a bow")
