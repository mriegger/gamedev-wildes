...
func _process(_delta: float) -> bool:
	...
	if _phase == 0 and _frame == 2:
		...
		_expect(sound_player.stream.resource_path == "res://assets/audio/sfx/crafting/complete/gavel_3_blows_s1590_CC0.ogg", "crafting used the wrong success sound")
		...
		_hud.crafting_panel._runes_tab.pressed.emit()
		_expect(_hud.crafting_panel.get_current_workspace_id() == &"runes", "Runes tab press did not change the workspace")
		_expect(_hud.inventory_ui_sound_player.stream == HUD.SECTION_NAVIGATION_SOUND, "Runes tab press did not play the section navigation sound")
		_hud.crafting_panel._progression_tab.pressed.emit()
		_expect(_hud.crafting_panel.get_current_workspace_id() == &"progression", "Progression tab press did not change the workspace")
		_expect(_hud.inventory_ui_sound_player.stream == HUD.SECTION_NAVIGATION_SOUND, "Progression tab press did not play the section navigation sound")
		_hud.crafting_panel._crafting_tab.pressed.emit()
		_expect(_hud.crafting_panel.get_current_workspace_id() == &"crafting", "Crafting tab press did not change the workspace")
		_expect(_hud.inventory_ui_sound_player.stream == HUD.SECTION_NAVIGATION_SOUND, "Crafting tab press did not play the section navigation sound")
		var recipe_button := _hud.crafting_panel._recipe_buttons[&"stone_pickaxe"] as Button
		recipe_button.pressed.emit()
		_expect(_hud.inventory_ui_sound_player.stream == HUD.DEFAULT_UI_CLICK_SOUND and _hud.inventory_ui_sound_player.playing, "recipe button press did not play the default UI click")
		_hud.inventory_ui_sound_player.stop()
		...
		_expect(_hud.crafting_panel._crafting_sound_streams.has(sound_player.stream), "successful craft used an unknown completion sound")
		_expect(sound_player.playing, "successful craft did not play its sound")
		var first_crafting_sound := sound_player.stream
		...
		_expect(sound_player.stream != first_crafting_sound, "consecutive crafts repeated the same completion sound")
...
func _check_open_state() -> void:
	...
	_expect(recipe_scroll != null and recipe_list.get_child_count() == 8, "scrollable recipe list did not contain eight general recipes")
